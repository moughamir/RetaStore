/*!
 * Screets Chat X - Console User Interface
 * Author: @screetscom
 *
...
;(function () {
	...
	function SCX_UI( opts ) {
		...
		// Common UI objects
		this.$btn_conn = $( '.scx-btn-conn' );
		this.$tab_head = D.getElementById( 'scx-tab-header' );
		this.$cnv = D.getElementById( 'scx-tab-cnv' ).querySelector('.scx-cnv');
		this.$sidebar_r = D.getElementById( 'scx-sidebar2' );
		this.$tab_ntf2 = ''; // It will be get on showing tab
		this.$settings = D.getElementById( 'scx-settings' );
	...
	SCX_UI.prototype = {
		...
		_ui : function() {
			...
			var self = this,
				$console_wrap = D.getElementById('scx-console'),
				$btn_logout = D.getElementById('scx-btn-logout'),
				$btn_reset = D.getElementById('scx-btn-reset');
			...
			if ( ( "Notification" in W ) && Notification.permission !== "granted" && Notification.permission !== 'denied' ) {
				...
				self.app.add_class( $('.scx-desk-ntf-alert'), 'scx-active' );
				
				var $turn_on = D.getElementById('scx-btn-turn-on');
				...
				$turn_on.addEventListener('click', function(e) {
					...
					Notification.requestPermission( function ( permission ) {
						...
						if ( permission === "granted" ) {
							...
							// Hide alert box
							 self.app.remove_class( $('.scx-desk-ntf-alert'), 'scx-active' );
			...
			$('.scx-main').addEventListener('click', function(e) {
			...
			// Sidebar tabs
			var $tabs_links = D.querySelectorAll('.scx-tabs li a'),
				$item = '',
				tab_id = '',
				$last_item = '';
			...
			for( var i=0; i < $tabs_links.length; i++ ) {
				...
				if( self.app.has_class( $item, 'scx-active') ) {

					self.app.add_class( D.getElementById(tab_id), 'scx-active' );
				...
				$item.addEventListener( 'click', function(e) {
					...
					// Hide last content
					self.app.remove_class( $last_item, 'scx-active' );
					self.app.remove_class( D.getElementById(last_tab_id), 'scx-active' );

					// Activate current content
					self.app.add_class( this, 'scx-active' );
					self.app.add_class( D.getElementById(tab_id), 'scx-active' );
			...
			D.getElementById('scx-btn-settings').addEventListener('click', function(e) {
				e.preventDefault();

				self.app.add_class( $('.scx-overlay'), 'scx-active' );
				self.app.add_class( self.$settings, 'scx-active' );

			});

			// Listen overlay clicks
			$( '.scx-overlay' ).addEventListener( 'click', function(e) {
				e.preventDefault();

				self.app.remove_class( this, 'scx-active' );
				self.app.remove_class( self.$settings, 'scx-active' );
			...
			var margin = 57,
				top_nav_h = $('.scx-top-nav').offsetHeight;
			...
			var fn_refresh_ui = function(e) {
				...
				D.getElementById('scx-sidebar').style.height = h_short + 'px';
				D.getElementById('scx-sidebar2').style.height = h + 'px';
				D.getElementById('scx-main').style.height = h + 'px';
		...
		_conn_btn : function( last_status ) {
			...
			if( last_status === 'offline' ) {
				...
				this.app.add_class( D.querySelector('body.wp-admin'), 'scx-online' );
			...
			// Go offline
				...
				this.app.remove_class( D.querySelector('body.wp-admin'), 'scx-online' );
			...
			// Get class 
			var cls = [ '_scx-', this.current_status ].join('');
			
			// Update database
			this.app.db.op_conn( this.current_status );

			// Update button UI
			removeClass.apply( $btn, ['_scx-connecting _scx-offline _scx-online', cls ] ); 
		...
		_on_connect : function( is_connected ) {
			...
			if( !is_connected ) {
				...
				removeClass.apply( this.$btn_conn, ['_scx-connecting _scx-online', '_scx-offline' ] ); 
		...
		_on_update_online : function( user ) {
			var self = this,
				item_id = [ 'scx-user-item-', user.id ].join(''),
		...
		_on_new_online : function( user_id, refreshing ) {
			...
			firebase.database().ref( 'users/' + user_id ).once( 'value' ).then( function( snap ) {
				...
				var $users_list = '',
					...
					list_item_id = 'scx-user-item-' + user.id,
				...
				if( is_you || user.is_op ) {
					...
					$users_list = D.getElementById( 'scx-ops' );
				...
				// 2. Chat user?
					...
					for( var chat_id in user.chats ) {
						cls.push('scx-chat-' + chat_id);
					}

					$users_list = D.getElementById( 'scx-visitors' );
				
				// 3. Web visitors
				} else {
					$users_list = D.getElementById( 'scx-web-visitors' );
		...
		_on_removed_online : function( user_id ) {
			...
			this.app.remove_obj( 'scx-user-item-' + user_id );
		...
		_show_tab : function( user, refreshing ) {
			...
			this.$tab_ntf2 = D.getElementById( 'scx-tab-ntf2' );
			
			var $reply = D.getElementById( 'scx-reply-' + user.id );
						...
						if( msg.length > 0 ) {
							...
							if( self.private_chats[user.id] ) {
								...
								self.app.db.create_chat( { name: self.app.username }, 'private', function( new_chat_id ) {
									...
									self.$cnv.id = 'scx-cnv-' + new_chat_id;
			...
			var $tab_nav = D.getElementById('scx-tab-nav-wrap'),
			...
			// 3. No chat...
				...
				self.app.ntf( 'main-ntf', self.opts._ntf_start_chat, 'warn', 'main', 'scx-ico-tip' );
			}

			// Get navigation			
			if( nav.length ) {
				$tab_nav.innerHTML = self.render( 'nav', { id: 'scx-header-nav', nav: nav.join(' ') });
		...
		_setup_list_item : function( $item ) {
			...
			if( $last_item ) {
				self.app.add_class( $last_item, 'scx-active' );
			}
			...
			$item.addEventListener( 'click', function(e) {
				...
				if( self.last_item_id ) {
					self.app.remove_class( D.getElementById(self.last_item_id), 'scx-active' );
				}
		...
		_tab_nav : function( tab ) {
			...
			var self = this,
				$btn_end_chat = D.getElementById('scx-btn-end-chat'),
				$btn_del = D.getElementById('scx-btn-delete');
			...
			if( $btn_end_chat ) {
				$btn_end_chat.addEventListener( 'click', function(e) {
					...
					if( ask ) {
						...
						if( tab === 'chat' ) {
							D.getElementById( 'scx-chat-item-' + self.current_chat_id ).click();
						}
				...
				if( this.chats[chat_id] ) {
					...
					if( !read_msgs[this.app.user_id] || read_msgs[this.app.user_id] < msg.timestamp ) {
						...
						if( ( msg.type === 'default' || msg.type === 'op2op' ) && msg.user_id !== this.app.user_id ) {
							...
							var class_name = ( msg.type === 'op2op' ) ? '.scx-user-' + msg.user_id : '.scx-chat-' + chat_id;

							// Highlight user
							this.app.add_class( $( class_name), 'scx-new-msg' );
		...
		_on_chat_history : function( chats ) {
			var self = this,
				$chats = D.getElementById('scx-chats');
			...
			var fn_show_chat = function(e) {
				...
				// Get navigation
				if( nav.length ) {
					chat._nav = self.render( 'nav', { id: 'scx-header-nav', nav: nav.join(' ') });
				} else {
					chat._nav = '';
				}
			...
			if( chats ) {
				...
				for( var chat_id in chats ) {
					...
					if( chats[chat_id].session ) {
						...
						$item = D.getElementById( 'scx-chat-item-' + chat_id );
        ...
        _on_update_setting : function( settings ) {
        	...
        	if( settings ) {
				...
				for( var name in settings ) {
					$field = D.getElementById( 'scx-f-' + name );
		...
		_read : function( chat_id, user_id ) {
			...
			setTimeout( function() {
				...
				if( $user_item ) {
					self.app.remove_class( $user_item, 'scx-new-msg' );
				}
		...
		_settings : function() {
			...
			this.app.add_class( $form, 'scx-active' );
	...
	SCX_UI.prototype.render = function( template, p ) {
		...
		switch( template ) {
			...
			// Notification box
			case 'ntf-box':
				arr = [ '<div class="scx-ntf-box"><div class="scx-wrap">',p.msg,p.links,'</div></div>' ];
				break;
			...
			// Chat history item
			case 'chat-item':
				arr = [ '<li id="scx-chat-item-',p.id,'" data-id="',p.id,'" class="scx-chat-', p.id, ' ', p._cls,'"><span class="scx-title">',p.name,'</span><span class="scx-time" data-livestamp="',(p.created_at/1000),'" title="',this.app.format_time( p.created_at, true ),'"></span></li>' ];
				break;

			// User list item
			case 'user-item':
				arr = [ '<li id="scx-user-item-',p.id,'" data-id="',p.id,'" class="scx-user-', p.id, ' ', p._cls,'"><span class="scx-icons">',p._timeago,p._ico,'</span><span class="scx-title">',p._flag,'', p.name, '</span>',p.meta,'</li>' ];
				break;

			case 'user-flag':
				arr = [ '<img class="scx-flag" src="',this.opts.plugin_url, '/assets/img/flags/',p,'.png" alt="" />' ];
				break;
			// User email
			case 'user-email':
				arr = [ '<a href="mailto:',p,'" class="scx-user-email">',p,'</a>' ];
				break;

			// User phone
			case 'user-phone':
				arr = [ '<a href="tel:',p,'" class="scx-user-phone">',p,'</a>' ];
				break;

			// User geo info
			case 'user-geo':
				arr = [ '<div class="scx-geo"><img class="scx-flag" src="',this.opts.plugin_url,'/assets/img/flags/',p.country_code,'.png" alt="" /> ',p.city,' ', p.country,'</div>' ];
				break;

			// User geo info (chat session style)
			case 'user-geo2':
				arr = [ '<div class="scx-geo"><img class="scx-flag" src="',this.opts.plugin_url,'/assets/img/flags/',p.country.code,'.png" alt="" /> ',p.city,' ', p.country.name,'</div>' ];
				break;

			// User platform info
			case 'user-platform':
				arr = [ '<div class="scx-platform">',p.browser,' ', p.browser_version,' - ', p.os, '<span class="scx-user-ip">',p.ip,'</span><span class="scx-looking-at">',this.opts._look_at,': <a href="',p.page_url,'" target="_blank">', p.page_url,'</a></span></div>' ];
				break;

			// User platform info (chat session style)
			case 'user-platform2':
				arr = [ '<div class="scx-platform">',p.browser,' ', p.browser_version,' - ', p.os, '<span class="scx-user-ip">',p.ip,'</span></div>' ];
				break;
			...
			// Vote note
			case 'vote':
				arr = [ '<div class="scx-vote scx-vote-',p.id,'"><i class="scx-ico-',p.id,'"></i>',p.v,'</div>' ];
				break;

			// User tab header
			case 'tab-header':
				arr = [ '<div class="scx-wrap"><div class="scx-title">',p.name,'</div><div class="scx-ntf scx-ntf-tab-header"></div><textarea id="scx-reply-',p.id,'" name="msg" class="scx-reply" placeholder="',this.opts._reply_ph,'" ',p._disable,'>',p._reply_v,'</textarea><div id="scx-tab-nav-wrap">',p._nav,'</div><div id="scx-tab-ntf2" class="scx-tab-ntf2"></div></div>' ];
				break;

			// User tab sidebar
			case 'tab-sidebar':
				arr = [ '<div class="scx-wrap">',p._avatar,'<div class="scx-title">',p.name,'</div><div class="scx-meta">',p._email,p._phone, p._geo, p._platform,'</div></div>' ];
				break;

			// Chat tab header
			case 'chat-tab-header':
				arr = [ '<div class="scx-wrap"><div class="scx-title">',p._title,'</div><div class="scx-subtitle">',p._subtitle,'</div><div class="scx-ntf scx-ntf-tab-header"></div><div id="scx-tab-nav-wrap">',p._nav,'</div></div>' ];
				break;

			// Chat tab sidebar
			case 'chat-tab-sidebar':
				arr = [ '<div class="scx-wrap">',p._avatar,'<div class="scx-title">',p.name,'</div><div class="scx-meta">',p._email,p._phone, p._geo, p._platform,p._vote,'</div></div>' ];
				break;
			...
			// Timeago
			case 'timeago':
				arr = [ '<abbr class="scx-time" data-livestamp="',(p/1000),'" title="',this.app.format_time( p, true ),'"></abbr>' ];
				break;
			...
			// Navigation link
			case 'nav-link':
				var _nav_link_cls = (p.desc) ? 'scx-tooltip' : '';
