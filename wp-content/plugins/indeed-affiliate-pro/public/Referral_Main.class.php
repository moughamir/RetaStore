...
class Referral_Main{
	...
	protected function set_affiliate_id(){
		...
		} else if (!empty($_COOKIE['uap_referral'])){ /// SEARCH INTO COOKIE
	...
	protected function save_referral_unverified($args=array()){
		...
		if ($referral_id){
			...
			if (get_option('uap_mlm_enable')){
				...
				$this->mlm_do_save_referral_unverified($args['affiliate_id'], $referral_id, 1, $limit, $args['amount'], $first_child_username, $referral_id);
			}		
		}
		return TRUE;
	}
