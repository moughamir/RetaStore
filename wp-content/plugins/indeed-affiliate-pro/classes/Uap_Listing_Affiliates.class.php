	...
	class Uap_Listing_Affiliates{
		...
		private function get_earnings_by_wpuid($uid=0){
			 ...
			 */
			 if ($uid){
			 	 global $wpdb;
				 $a = $wpdb->base_prefix . 'uap_affiliates';
			 	 $r = $wpdb->base_prefix . 'uap_referrals';
		...
		private function get_total_affiliates($ranks=array()){
			...
			$a = $wpdb->base_prefix . 'uap_affiliates';
			$q = "SELECT COUNT(a.id) as v FROM $a a WHERE 1=1";
			if ($ranks){
			 ...
			 */
			 ...
			 switch ($order_by){
				...
				case 'random':
					...
					$q = "SELECT a.id FROM $u u INNER JOIN $a a ON a.uid=u.ID WHERE 1=1";
					if ($ranks){
