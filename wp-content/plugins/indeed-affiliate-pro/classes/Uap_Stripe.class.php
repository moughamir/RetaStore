...
class Uap_Stripe{
	...
	public function do_payout($uid=0, $affiliate_id=0, $amount=0, $currency='usd'){
		...
		if (empty($metas['uap_affiliate_stripe_name']) || empty($metas['uap_affiliate_stripe_card_number'])
			|| empty($metas['uap_affiliate_stripe_cvc']) || empty($metas['uap_affiliate_stripe_expiration_month'])
			|| empty($metas['uap_affiliate_stripe_expiration_year']) || empty($metas['uap_affiliate_stripe_card_type'])
		) return '';
