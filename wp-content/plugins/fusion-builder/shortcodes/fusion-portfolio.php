		...
		public function render( $args, $content = '' ) {
			...
			$defaults = apply_filters(
				...
				FusionBuilder::set_shortcode_defaults(
					array(
						...
						'class'                    => '',
						'column_spacing'           => ( class_exists( 'Avada' ) ) ? Avada()->settings->get( 'portfolio_column_spacing' ) : '',
						'columns'                  => 3,
						'content_length'           => 'excerpt',
						'excerpt_length'           => ( class_exists( 'Avada' ) ) ? Avada()->settings->get( 'excerpt_length_portfolio' ) : '',
						...
						'layout'                   => 'carousel',
						'mouse_scroll'             => 'no',
						'number_posts'             => ( class_exists( 'Avada' ) ) ? Avada()->settings->get( 'portfolio_items' ) : '',
						'offset'                   => '',
			...
			if ( 'default' === $defaults['picture_size'] ) {
				if ( class_exists( 'Avada' ) ) {
					$image_size = Avada()->settings->get( 'portfolio_featured_image_size' );
			...
			if ( ! FusionBuilder::get_theme_option( 'featured_image_placeholder' ) ) {
			...
			if ( 'carousel' == $layout ) {
				...
				if ( $portfolio_categories && 'no' != $filters ) {
					...
					if ( 'yes-without-all' != $filters ) {
						$filter = '<li ' . FusionBuilder::attributes( 'fusion-filter fusion-filter-all fusion-active' ) . '><a ' . FusionBuilder::attributes( 'portfolio-shortcode-filter-link', array( 'data-filter' => '*' ) ) . '>' . esc_attr__( 'All', 'fusion-core' ) . '</a></li>';
						$first_filter = false;
					}
