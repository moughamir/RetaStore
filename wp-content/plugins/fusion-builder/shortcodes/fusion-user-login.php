...
class FusionSC_Login {
	...
	function render_login( $args, $content = '' ) {
		...
		if ( ! is_user_logged_in() ) {
			...
			$html .= '<label class="fusion-hidden-content" for="user_login">' . esc_attr__( 'Username', 'fusion-core' ) . '</label>';
			$html .= '<input type="text" name="log" placeholder="' . esc_attr__( 'Username', 'fusion-core' ) . '" value="' . esc_attr( $user_login ) . '" size="20" class="fusion-login-username input-text" id="user_login" />';
			$html .= '</div>';

			$html .= '<div class="fusion-login-input-wrapper">';
			$html .= '<label class="fusion-hidden-content" for="user_pass">' . esc_attr__( 'Password', 'fusion-core' ) . '</label>';
			$html .= '<input type="password" name="pwd" placeholder="' . esc_attr__( 'Password', 'fusion-core' ) . '" value="" size="20" class="fusion-login-password input-text" id="user_pass" />';
			$html .= '</div>';

			$html .= '<div class="fusion-login-submit-wrapperr">';
			$html .= '<button ' . FusionBuilder::attributes( 'login-shortcode-button' ) . '>' . esc_attr__( 'Log in', 'fusion-core' ) . '</button>';
			...
			$html .= '<a class="fusion-login-lost-passowrd" target="_self" href="' . $lost_password_link . '">' . esc_attr__( 'Lost password?', 'fusion-core' ) . '</a>';
			$html .= '<a class="fusion-login-register" target="_self" href="' . $register_link . '">' . esc_attr__( 'Register', 'fusion-core' ) . '</a>';
			$html .= '</div>';
			...
			$html .= '<div class="fusion-login-caption">' . sprintf( esc_attr__( 'Welcome %s', 'fusion-core' ), ucwords( $user->display_name ) ) . '</div>';
			...
			$html .= '<li><a href="' . get_dashboard_url() . '">' . esc_attr__( 'Dashboard', 'fusion-core' ) . '</a></li>';
			$html .= '<li><a href="' . get_edit_user_link( $user->ID ) . '">' . esc_attr__( 'Profile', 'fusion-core' ) . '</a></li>';
			$html .= '<li><a href="' . wp_logout_url( get_permalink() ) . '">' . esc_attr__( 'Logout', 'fusion-core' ) . '</a></li>';
			$html .= '</ul>';
	...
	function render_register( $args, $content = '' ) {
		...
		if ( ! is_user_logged_in() ) {
			...
			$html .= '<div class="fusion-login-input-wrapper">';
			$html .= '<label class="fusion-hidden-content" for="user_login">' . esc_attr__( 'Username', 'fusion-core' ) . '</label>';
			$html .= '<input type="text" name="user_login" placeholder="' . esc_attr__( 'Username', 'fusion-core' ) . '" value="" size="20" class="fusion-login-username input-text" id="user_login" />';
			$html .= '</div>';

			$html .= '<div class="fusion-login-input-wrapper">';
			$html .= '<label class="fusion-hidden-content" for="user_pass">' . esc_attr__( 'Email', 'fusion-core' ) . '</label>';
			$html .= '<input type="text" name="user_email" placeholder="' . esc_attr__( 'Email', 'fusion-core' ) . '" value="" size="20" class="fusion-login-email input-text" id="user_email" />';
			$html .= '</div>';
			...
			$html .= '<p class="fusion-login-input-wrapper">' . esc_attr__( 'Registration confirmation will be e-mailed to you.', 'fusion-core' ) . '</p>';

			$html .= '<div class="fusion-login-submit-wrapperr">';
			$html .= '<button ' . FusionBuilder::attributes( 'login-shortcode-button' ) . '>' . esc_attr__( 'Register', 'fusion-core' ) . '</button>';
			...
			$html .= do_shortcode( '[fusion_alert type="general" border_size="1px" box_shadow="yes"]' . esc_attr__( 'You are already signed up.', 'fusion-core' ) . '[/fusion_alert]' );
		}
	...
	function render_lost_password( $args, $content = '' ) {
		...
		if ( ! is_user_logged_in() ) {
			...
			$html .= '<p class="fusion-login-input-wrapper">' . esc_attr__( 'Lost your password? Please enter your username or email address. You will receive a link to create a new password via email.', 'fusion-core' ) . '</p>';

			$html .= '<div class="fusion-login-input-wrapper">';
			$html .= '<label class="fusion-hidden-content" for="user_login">' . esc_attr__( 'Username or Email', 'fusion-core' ) . '</label>';
			$html .= '<input type="text" name="user_login" placeholder="' . esc_attr__( 'Username or Email', 'fusion-core' ) . '" value="" size="20" class="fusion-login-username input-text" id="user_login" />';
			$html .= '</div>';

			$html .= '<div class="fusion-login-submit-wrapperr">';
			$html .= '<button ' . FusionBuilder::attributes( 'login-shortcode-button' ) . '>' . esc_attr__( 'Reset Password', 'fusion-core' ) . '</button>';
			...
			$html .= do_shortcode( '[fusion_alert type="general" border_size="1px" box_shadow="yes"]' . esc_attr__( 'You are already signed in.', 'fusion-core' ) . '[/fusion_alert]' );
		}
	...
	public function render_notices( $context = '' ) {
		...
		if ( isset( $_GET['action'] ) && $_GET['action'] == $context ) {
			...
			if ( 'login' == $_GET['action'] && isset( $_GET['success'] ) && '0' == $_GET['success'] ) {
				$notice_type = 'error';
				$notices     = esc_attr__( 'Login failed, please try again.', 'fusion-core' );
				// Registration.
			} elseif ( 'register' == $_GET['action'] ) {
				// Success.
				if ( isset( $_GET['success'] ) && '1' == $_GET['success'] ) {
					$notice_type = 'success';
					$notices     = esc_attr__( 'Registration complete. Please check your e-mail.', 'fusion-core' );
					// Error.
				} else {
					$notice_type = 'error';
					$notices = '';
					...
					if ( isset( $_GET['empty_username'] ) ) {
						$notices .= esc_attr__( 'Please enter a username.', 'fusion-core' ) . '<br />';
					}
					// Empty email.
					if ( isset( $_GET['empty_email'] ) ) {
						$notices .= esc_attr__( 'Please type your e-mail address.', 'fusion-core' ) . '<br />';
					}
					// Username exists.
					if ( isset( $_GET['username_exists'] ) ) {
						$notices .= esc_attr__( 'This username is already registered. Please choose another one.', 'fusion-core' ) . '<br />';
					}
					// Email exists.
					if ( isset( $_GET['email_exists'] ) ) {
						$notices .= esc_attr__( 'This email is already registered, please choose another one.', 'fusion-core' ) . '<br />';
					}

					// Generic Error.
					if ( ! $notices ) {
						$notices .= esc_attr__( 'Something went wrong during registration. Please try again.', 'fusion-core' );
				...
				if ( isset( $_GET['success'] ) && '1' == $_GET['success'] ) {
					$notice_type = 'success';
					$notices     = esc_attr__( 'Check your e-mail for the confirmation link.', 'fusion-core' );
					// Error.
				} else {
					$notice_type = 'error';
					$notices     = '';
					...
					if ( isset( $_GET['empty_login'] ) ) {
						$notices .= esc_attr__( 'Enter a username or e-mail address.', 'fusion-core' ) . '<br />';
					}

					// Empty login.
					if ( isset( $_GET['unregisred_user'] ) ) {
						$notices .= esc_attr__( 'Invalid username.', 'fusion-core' ) . '<br />';
					}

					// Empty login.
					if ( isset( $_GET['unregistered_mail'] ) ) {
						$notices .= esc_attr__( 'There is no user registered with that email address.', 'fusion-core' ) . '<br />';
					}

					// Generic Error.
					if ( ! $notices ) {
						$notices .= esc_attr__( 'Invalid username or e-mail.', 'fusion-core' );
						// Delete the last line break.
					} else {
						$notices = substr( $notices, 0, strlen( $notices ) - 6 );
					}
	...
	fusion_builder_map( array(
		...
		'params'      => array(
			array(
				...
				'description' => esc_attr__( 'Choose the alignment of all content parts. "Text Flow" follows the default text align of the site. "Center" will center all elements.', 'fusion-core' ),
