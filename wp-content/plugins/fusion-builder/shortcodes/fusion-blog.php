...
class FusionSC_Blog {
	...
	public function grid_timeline_comments() {
		...
		if ( self::$args['meta_comments'] ) {
			...
			$comments = '<i class="fusion-icon-bubbles"></i>&nbsp;' . esc_attr__( 'Protected', 'fusion-core' );
	...
	public function read_more() {
		...
		if ( self::$args['meta_link'] ) {
			...
			if ( self::$args['meta_read'] ) {
				...
				$inner_content .= apply_filters( 'avada_read_more_name', esc_attr__( 'Read More', 'fusion-core' ) );
				$inner_content .= '</a>';
				$inner_content .= '</div>';
